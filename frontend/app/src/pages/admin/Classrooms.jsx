...
const AdminClassrooms = () => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await api.put(`/classrooms/${editingClassroom.id}`, formData);
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nome da Sala</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {classrooms.map((classroom) => (
              <tr key={classroom.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{classroom.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{classroom.type || "-"}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{classroom.capacity} lugares</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${classroom.is_available ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                    {classroom.is_available ? "Disponível" : "Indisponível"}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <button onClick={() => handleEditClick(classroom)} className="text-indigo-600 hover:text-indigo-900 mr-4">Editar</button>
                  <button onClick={() => handleDeleteClick(classroom)} className="text-red-600 hover:text-red-900">Apagar</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {editingClassroom && (
        <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-xl w-96">
            <h2 className="text-xl font-bold mb-4">Editar Sala #{editingClassroom.id}</h2>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setCreateFormData({ ...createFormData, capacity: parseInt(e.target.value) || 0 })}
                  className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                />
              </div>
              <div className="mb-4">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setCreateFormData({ ...createFormData, is_available: e.target.checked })}
                    className="mr-2"
                  />
                  <span className="text-sm">Sala Disponível</span>
                </label>
        ...
        <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm overflow-y-auto h-full w-full flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-96 transform transition-all scale-100">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">Eliminar Sala</h3>
              <p className="text-sm text-gray-500 mb-6">
                Tens a certeza que queres eliminar a sala <span className="font-bold text-gray-800">{classroomToDelete.name}</span>? <br />
